...
impl CLI {
    ...
    pub fn publish_parse(&self) -> PublishCommand {
        ...
        opts.optopt("t",
                    "",
                    "The topic name which payload data is published.",
                    "topic");
        ...
        opts.optopt("k",
                    "",
                    "Keep alive the link with the server then try to send ping request.
Defaults \
                     to 60",
                    "seconds");
        ...
        opts.optopt("u",
                    "",
                    "Specifies a username with which to authenticate to",
                    "username");
        opts.optopt("P",
                    "",
                    "Specifies a password with which to authenticate to",
                    "password");
        opts.optopt("v",
                    "",
                    "MQTT protocol version. Can be 3.1 or 3.1.1",
                    "version");
        ...
        let topic = matches.opt_str("t")
            .unwrap_or_else(|| self.cli_error("Please set the topic name"));
    ...
    pub fn subscribe_parse(&self) -> SubscribeCommand {
        ...
        opts.optopt("k",
                    "",
                    "Keep alive the link with the server then try to send ping request. Defaults \
                     to 60",
                    "seconds");
        ...
        // opts.optopt("f", "", "Piece of topic path to filter out incomming messages.
        // Can be repeated.", "filter");
    ...
    fn print_usage(&self) {
        let mut brief = "mqttc is a simple MQTT client that provides to publish message or \
                         subscribe to topics.\n\n"
            .to_string();
        brief = brief +
                format!("Usage:\n    {} command\n    {} --help\n\n",
